...
//
//  Edited by Rastislav Budinsky on 8/31/22.
...
Block::Block(const Block *parent_, const Miner *miner_, BlockTime timeSeconds_, Value txFees, BlockHeight height_, Value txFeesInChain_, Value valueInChain_) : timeBroadcast(timeMax), parent(parent_), miner(miner_), height(height_), timeMined(timeSeconds_), value(txFees), txFeesInChain(txFeesInChain_), valueInChain(valueInChain_) {}
...
Block::Block(const Block *parent_, const Miner *miner_, BlockTime timeSeconds_, Value txFees) :
    ...
    Block(parent_, miner_, timeSeconds_, txFees, parent_->height + BlockHeight(1), parent_->txFeesInChain + txFees, parent_->valueInChain + txFees) {}
...
void Block::reset(const Block *parent_, const Miner *miner_, BlockTime timeSeconds_, Value txFees) {
    ...
    valueInChain = parent->valueInChain + txFees;
